...
// Local Subcomponents within sidebar/
import { SidebarHeader } from "./sidebar-header";
import { SidebarSearch } from "./sidebar-search";
import { SidebarActiveUsers, type ActiveUser } from "./sidebar-active-users";
import { SidebarFooter } from "./sidebar-footer";
import { UserResultItem } from "./user-result-item";

// Conversations Subcomponents from conversations/
import { ConversationItem } from "../conversations/conversation-item";
import { ConversationSkeleton } from "../conversations/conversation-skeleton";
import { EmptyConversations } from "../conversations/empty-conversations";
import { ConversationsError } from "../conversations/conversations-error";

// Types
import type { Conversation, SearchedUser } from "@/types/conversation";
...
export function ChatSidebar({ onNewChatClick, onToggleSidebar }: ChatSidebarProps) {
  ...
  const handleSelectConversation = useCallback(
    (id: string) => dispatch(setActiveConversationId(id)),
    [dispatch]
  );

  const handleSelectActiveUser = useCallback(
    (u: ActiveUser) => {
      if (u.conversationId) dispatch(setActiveConversationId(u.conversationId));
    },
    [dispatch]
  );
  ...
  return (
    <div className="w-80 sm:w-84 h-full bg-[#FFEED6] dark:bg-card border-r border-brand-dark/15 dark:border-white/10 sidebar-right-shadow flex flex-col justify-between select-none">
      ...
      <div className="p-4 space-y-4">
        ...
        {!hasQuery && (
          <SidebarActiveUsers users={activeUsers} onSelectUser={handleSelectActiveUser} />
        )}
      </div>
